...
export async function saveMenuItem(_prev: FormState, fd: FormData): Promise<FormState> {
  ...
  const parsed = menuItemSchema.safeParse({
    ...
    price: str(fd, "price"),
    fullPrice: str(fd, "fullPrice"),
    baseLabel: str(fd, "baseLabel") || "Normal",
    image: str(fd, "image"),
...
const settingsSchema = z.object({
  ...
  isOpen: z.boolean(),
  autoHours: z.boolean(),
  openTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, "Opening time must look like 10:00"),
  closeTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, "Closing time must look like 22:00"),
...
export async function saveSettings(_prev: FormState, fd: FormData): Promise<FormState> {
  ...
  const parsed = settingsSchema.safeParse({
    ...
    autoHours: bool(fd, "autoHours"),
    openTime: str(fd, "openTime"),
    closeTime: str(fd, "closeTime"),
